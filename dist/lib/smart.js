...
function getSecurityExtensionsFromConformanceStatement(baseUrl = "/", requestOptions) {
    return (0, lib_1.fetchConformanceStatement)(baseUrl, requestOptions).then(meta => {
        ...
        const extensions = ((0, lib_1.getPath)(meta || {}, "rest.0.security.extension") || [])
...
async function authorize(env, params = {}) {
    ...
    // ------------------------------------------------------------------------
    // Obtain input
    const { clientSecret, fakeTokenResponse, encounterId, target, width, height, pkceMode, clientPublicKeySetUrl, refreshTokenWithCredentials } = params;
    ...
    if (iss) {
        (0, debug_1.debug)("Making %s launch...", launch ? "EHR" : "standalone");
    }
    ...
    if (fhirServiceUrl && !iss) {
        (0, debug_1.debug)("Making fake launch...");
    ...
    if (target && isBrowser()) {
        ...
        if (win !== self) {
            try {
                ...
                (0, debug_1.debug)(`oauth2: Failed to modify window.sessionStorage. Perhaps it is from different origin?. Failing back to "_self". %s`, ex);
                win = self;
            }
        }
        if (win !== self) {
            try {
                ...
                (0, debug_1.debug)(`oauth2: Failed to modify window.location. Perhaps it is from different origin?. Failing back to "_self". %s`, ex);
...
async function ready(env, options = {}) {
    ...
    (0, debug_1.debug)("key: %s, code: %s", key, code);
    ...
    if (isBrowser() && (code || hasState)) {
        ...
        if (code) {
            params.delete("code");
            (0, debug_1.debug)("Removed code parameter from the url.");
        }
        // We no longer need the `state` key. It will be stored to a well know
        // location - sessionStorage[SMART_KEY]
        if (hasState) {
            params.delete("state");
            (0, debug_1.debug)("Removed state parameter from the url.");
        }
    ...
    if (!authorized && state.tokenUri) {
        ...
        (0, debug_1.debug)("Preparing to exchange the code for access token...");
        ...
        (0, debug_1.debug)("Authorization successful!");
    }
    else {
        (0, debug_1.debug)(((_b = state.tokenResponse) === null || _b === void 0 ? void 0 : _b.access_token) ?
            "Already authorized" :
            "No authorization needed");
    }
    await Storage.set(settings_1.SMART_KEY, key);
    const client = new Client_1.default(env, state);
    (0, debug_1.debug)("Created client instance: %O", client);
    return client;
}
exports.ready = ready;
/**
...
async function buildTokenRequest(env, { code, state, clientPublicKeySetUrl, privateKey }) {
    ...
    if (clientSecret) {
        requestOptions.headers.authorization = "Basic " + env.base64encode(clientId + ":" + clientSecret);
        (0, debug_1.debug)("Using state.clientSecret to construct the authorization header: %s", requestOptions.headers.authorization);
    }
    // Asymmetric auth
    else if (privateKey) {
        ...
        (0, debug_1.debug)("Using state.clientPrivateJwk to add a client_assertion to the POST body");
    }
    // Public client
    else {
        (0, debug_1.debug)("Public client detected; adding state.clientId to the POST body");
        requestOptions.body += `&client_id=${encodeURIComponent(clientId)}`;
    }
    if (codeVerifier) {
        (0, debug_1.debug)("Found state.codeVerifier, adding to the POST body");
