...
exports.init = exports.buildTokenRequest = exports.ready = exports.authorize = void 0;
...
async function authorize(env, params = {}) {
    ...
    if (Array.isArray(params)) {
        ...
        (0, isomorphic_1.assert)(cfg, `No configuration found matching the current "iss" parameter "${urlISS}"`);
    ...
    if (iss) {
        (0, isomorphic_1.debug)("Making %s launch...", launch ? "EHR" : "standalone");
    }
    ...
    if (isBrowser()) {
        const inFrame = (0, browser_1.isInFrame)();
        const inPopUp = (0, browser_1.isInPopUp)();
    ...
    const stateKey = (0, isomorphic_1.randomString)(16);
    const state = {
        clientId,
        scope,
        redirectUri,
    ...
    if (fhirServiceUrl && !iss) {
        (0, isomorphic_1.debug)("Making fake launch...");
    ...
    const extensions = await (0, isomorphic_1.getSecurityExtensions)(serverUrl);
    ...
    if ((0, isomorphic_1.shouldIncludeChallenge)(extensions.codeChallengeMethods.includes('S256'), pkceMode)) {
    ...
    if (target && isBrowser()) {
        let win;
        win = await (0, browser_1.getTargetWindow)(target, width, height);
        if (win !== self) {
            try {
                ...
                (0, isomorphic_1.debug)(`oauth2: Failed to modify window.sessionStorage. Perhaps it is from different origin?. Failing back to "_self". %s`, ex);
                win = self;
            }
        }
        if (win !== self) {
            try {
                ...
                self.addEventListener("message", browser_1.onMessage);
            }
            catch (ex) {
                (0, isomorphic_1.debug)(`oauth2: Failed to modify window.location. Perhaps it is from different origin?. Failing back to "_self". %s`, ex);
...
async function ready(env, options = {}) {
    ...
    (0, isomorphic_1.debug)("key: %s, code: %s", key, code);
    // key might be coming from the page url so it might be empty or missing
    (0, isomorphic_1.assert)(key, "No 'state' parameter found. Please (re)launch the app.");
    ...
    if (isBrowser() && (code || hasState)) {
        ...
        if (code) {
            params.delete("code");
            (0, isomorphic_1.debug)("Removed code parameter from the url.");
        }
        // We no longer need the `state` key. It will be stored to a well know
        // location - sessionStorage[SMART_KEY]
        if (hasState) {
            params.delete("state");
            (0, isomorphic_1.debug)("Removed state parameter from the url.");
        }
    ...
    (0, isomorphic_1.assert)(state, "No state found! Please (re)launch the app.");
    ...
    if (!authorized && state.tokenUri) {
        (0, isomorphic_1.assert)(code, "'code' url parameter is required");
        (0, isomorphic_1.debug)("Preparing to exchange the code for access token...");
        ...
        (0, isomorphic_1.debug)("Token request options: %O", requestOptions);
        // The EHR authorization server SHALL return a JSON structure that
        // includes an access token or a message indicating that the
        // authorization request has been denied.
        const tokenResponse = await (0, isomorphic_1.request)(state.tokenUri, requestOptions);
        (0, isomorphic_1.debug)("Token response: %O", tokenResponse);
        (0, isomorphic_1.assert)(tokenResponse.access_token, "Failed to obtain access token.");
        // Now we need to determine when is this authorization going to expire
        state.expiresAt = (0, isomorphic_1.getAccessTokenExpiration)(tokenResponse);
        // save the tokenResponse so that we don't have to re-authorize on
        // every page reload
        state = { ...state, tokenResponse };
        await Storage.set(key, state);
        (0, isomorphic_1.debug)("Authorization successful!");
    }
    else {
        (0, isomorphic_1.debug)(((_b = state.tokenResponse) === null || _b === void 0 ? void 0 : _b.access_token) ?
            "Already authorized" :
            "No authorization needed");
    }
    await Storage.set(settings_1.SMART_KEY, key);
    const client = new Client_1.default(state, env.getStorage());
    (0, isomorphic_1.debug)("Created client instance: %O", client);
    return client;
}
exports.ready = ready;
/**
...
async function buildTokenRequest(env, { code, state, clientPublicKeySetUrl, privateKey }) {
    ...
    (0, isomorphic_1.assert)(redirectUri, "Missing state.redirectUri");
    (0, isomorphic_1.assert)(tokenUri, "Missing state.tokenUri");
    (0, isomorphic_1.assert)(clientId, "Missing state.clientId");
    ...
    if (clientSecret) {
        requestOptions.headers.authorization = "Basic " + (0, isomorphic_1.base64encode)(clientId + ":" + clientSecret);
        (0, isomorphic_1.debug)("Using state.clientSecret to construct the authorization header: %s", requestOptions.headers.authorization);
    }
    // Asymmetric auth
    else if (privateKey) {
        ...
        const jwtClaims = {
            ...
            exp: (0, isomorphic_1.getTimeInFuture)(120) // two minutes in the future
        };
        ...
        (0, isomorphic_1.debug)("Using state.clientPrivateJwk to add a client_assertion to the POST body");
    }
    // Public client
    else {
        (0, isomorphic_1.debug)("Public client detected; adding state.clientId to the POST body");
        requestOptions.body += `&client_id=${encodeURIComponent(clientId)}`;
    }
    if (codeVerifier) {
        (0, isomorphic_1.debug)("Found state.codeVerifier, adding to the POST body");
...
async function init(env, authorizeOptions, readyOptions) {
    ...
    if (cached) {
        return new Client_1.default(cached, env.getStorage());
    }
