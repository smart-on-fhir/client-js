...
class Client {
  ...
  constructor(environment, state) {
    ...
    this.forCapability = async capability => {
      var _a, _b;
      const smartConfiguration = await (0, smart_1.fetchWellKnownJson)(client.state.serverUrl);
      const options = (smartConfiguration.capabilities.includes(capability) ? [client.state.serverUrl] : []).concat(((smartConfiguration === null || smartConfiguration === void 0 ? void 0 : smartConfiguration.associated_endpoints) || []).filter(e => e.capabilities.includes(capability)).map(e => e.url));
      const defaultUrl = (_b = (_a = client.state.associatedEndpointDefaults) === null || _a === void 0 ? void 0 : _a.find(e => e.capabilities.includes(capability))) === null || _b === void 0 ? void 0 : _b.url;
      if (options.length === 0 && !defaultUrl || options.length > 1) {
        throw `Expected 1 endpoint for ${capability} but found ${options.length}: ${options}`;
      }
      return new Client(client.environment, Object.assign(Object.assign({}, client.state), {
