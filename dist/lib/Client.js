...
function resolveRefs(obj, fhirOptions, cache, client, requestOptions) {
    ...
    let paths = (0, lib_1.makeArray)(fhirOptions.resolveReferences)
    ...
    paths = paths.filter((p, i) => {
        ...
        if (index > -1) {
            (0, debug_1.debug)("Client: Duplicated reference path \"%s\"", p);
            return false;
        }
        return true;
    });
...
class Client {
    ...
    getPatientId() {
        ...
        if (tokenResponse) {
            ...
            if (!tokenResponse.patient) {
                if (!(this.state.scope || "").match(/\blaunch(\/patient)?\b/)) {
                    (0, debug_1.debug)("Client: " + strings_1.default.noScopeForId, "patient", "patient");
                }
                else {
                    // The server should have returned the patient!
                    (0, debug_1.debug)("Client: The ID of the selected patient is not available. Please check if your server supports that.");
                }
                return null;
            }
        ...
        if (this.state.authorizeUri) {
            (0, debug_1.debug)("Client: " + strings_1.default.noIfNoAuth, "the ID of the selected patient");
        }
        else {
            (0, debug_1.debug)("Client: " + strings_1.default.noFreeContext, "selected patient");
        }
        return null;
    }
    /**
    ...
    getEncounterId() {
        ...
        if (tokenResponse) {
            ...
            if (!tokenResponse.encounter) {
                if (!(this.state.scope || "").match(/\blaunch(\/encounter)?\b/)) {
                    (0, debug_1.debug)("Client: " + strings_1.default.noScopeForId, "encounter", "encounter");
                }
                else {
                    // The server should have returned the encounter!
                    (0, debug_1.debug)("Client: The ID of the selected encounter is not available. Please check if your server supports that, and that the selected patient has any recorded encounters.");
                }
                return null;
            }
        ...
        if (this.state.authorizeUri) {
            (0, debug_1.debug)("Client: " + strings_1.default.noIfNoAuth, "the ID of the selected encounter");
        }
        else {
            (0, debug_1.debug)("Client: " + strings_1.default.noFreeContext, "selected encounter");
        }
        return null;
    }
    /**
    ...
    getIdToken() {
        ...
        if (tokenResponse) {
            ...
            if (!idToken) {
                ...
                if (!hasOpenid || !(hasFhirUser || hasProfile)) {
                    (0, debug_1.debug)("Client: You are trying to get the id_token but you are not " +
                    ...
                    (0, debug_1.debug)("Client: The id_token is not available. Please check if your server supports that.");
                }
                return null;
            }
        ...
        if (this.state.authorizeUri) {
            (0, debug_1.debug)("Client: " + strings_1.default.noIfNoAuth, "the id_token");
        }
        else {
            (0, debug_1.debug)("Client: " + strings_1.default.noFreeContext, "id_token");
        }
        return null;
    }
    /**
    ...
    getAuthorizationHeader() {
        ...
        if (username && password) {
            return "Basic " + this.environment.base64encode(username + ":" + password);
        }
        return null;
    }
    /**
    ...
    async request(requestOptions, fhirOptions = {}, _resolvedRefs = {}) {
        ...
        return job
            ...
            // Make the request
            .then(requestOptions => {
            (0, debug_1.debug)("Client.request: %s, options: %O, fhirOptions: %O", url, requestOptions, options);
            ...
            if (error.status == 401) {
                ...
                if (!options.useRefreshToken) {
                    (0, debug_1.debug)("Client.request: Your session has expired and the useRefreshToken option is set to false. Please re-launch the app.");
                ...
                // Need to re-launch. Clear state to start over!
                (0, debug_1.debug)("Client.request: Auto-refresh failed! Please re-launch the app.");
                await this._clearState();
                error.message += "\n" + strings_1.default.expired;
                throw error;
            }
            ...
            .catch((error) => {
            if (error.status == 403) {
                (0, debug_1.debug)("Client.request: Permission denied! Please make sure that you have requested the proper scopes.");
            }
            throw error;
        })
            .then((data) => {
    ...
    refresh(requestOptions = {}) {
        var _a, _b;
        (0, debug_1.debug)("Client.refresh: Attempting to refresh with refresh_token...");
        ...
        if (!this._refreshTask) {
            ...
            if (!("authorization" in refreshRequestOptions.headers)) {
                ...
                if (clientSecret) {
                    // @ts-ignore
                    refreshRequestOptions.headers.authorization = "Basic " + this.environment.base64encode(clientId + ":" + clientSecret);
                }
            }
            this._refreshTask = (0, lib_1.request)(tokenUri, refreshRequestOptions)
                ...
                (0, debug_1.debug)("Client.refresh: Received new access token response %O", data);
                ...
                if ((_b = (_a = this.state) === null || _a === void 0 ? void 0 : _a.tokenResponse) === null || _b === void 0 ? void 0 : _b.refresh_token) {
                    (0, debug_1.debug)("client:refresh: Deleting the expired or invalid refresh token.");
                ...
                if (key) {
                    ...
                    (0, debug_1.debug)("Client.refresh: No 'key' found in Clint.state. Cannot persist the instance.");
                }
            });
        }
     ...
     */
    getFhirVersion() {
        return (0, lib_1.fetchConformanceStatement)(this.state.serverUrl)
