 ...
 */
export async function fetchWellKnownJson(baseUrl = "/", requestOptions) {
 ...
 */
export async function getSecurityExtensions(baseUrl = "/", wellKnownRequestOptions, conformanceRequestOptions) {
 ...
 * @param [params]
 */
export async function authorize(env, params = {}) {
    ...
    // ------------------------------------------------------------------------
    // Obtain input
    const { clientSecret, fakeTokenResponse, encounterId, target, width, height, pkceMode, clientPublicKeySetUrl, 
    // Two deprecated values to use as fall-back values later
    redirect_uri, client_id, } = params;
    let { iss, launch, patientId, fhirServiceUrl, redirectUri, noRedirect, scope = "", clientId, completeInTarget, clientPrivateJwk, stateKey } = params;
 ...
 */
export async function ready(env, options = {}) {
    const url = env.getUrl();
    const Storage = env.getStorage();
 ...
 */
export async function buildTokenRequest(env, { code, state, clientPublicKeySetUrl, privateKey }) {
 ...
 */
export async function init(env, authorizeOptions, readyOptions) {
    const url = env.getUrl();
