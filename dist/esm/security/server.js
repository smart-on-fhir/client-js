import { base64url, SignJWT, importJWK as joseImportJWK } from "jose";
import { randomBytes, createHash } from "crypto";
export { randomBytes };
export async function digestSha256(payload) {
    const hash = createHash('sha256');
    hash.update(payload);
    return hash.digest();
}
export async function generatePKCEChallenge(entropy = 96) {
...
export async function importJWK(jwk) {
    ...
    return joseImportJWK(jwk);
}
export async function signCompactJws(alg, privateKey, header, payload) {
