...
/***/ function(module, exports, __webpack_require__) {
	...
	(function() {
	    ...
	    var currentRequests = [];
	    var buffer = [];
	    var inRefreshRequest = false;

		function _addToBuffer (settings, deferred) {
		    buffer.push({
		        settings: settings,
		        deferred: deferred
		    });
		...
		// function to call to refresh token
		function _tokenExpiration () {
            ...
            jquery.ajax(opts)
                ...
                .fail(function(err) {d.reject({error: err, data: err, config: args});});
			return d.promise();
		}
		...
		function _runBuffer(token) {
			...
			for (var i in buffer) {
				...
				buffer[i].settings.retrying = true;
				buffer[i].settings.headers["Authorization"] = 'Bearer ' + token;
			...
			$.when.apply($, promises)
				.done(function () {
					inRefreshRequest = false;
				})
				.fail(function () {
					inRefreshRequest = false;
					throw new Error('Running buffer of requests failed');
				});
		}
	                ...
	                withCredentials: args.credentials === 'include',
							...
							jquery.ajaxPrefilter(function (opts, originalOpts, jqxhr) {
								...
								jqxhr.always(function () {
									currentRequests.splice($.inArray(opts.url, currentRequests), 1);
								});
